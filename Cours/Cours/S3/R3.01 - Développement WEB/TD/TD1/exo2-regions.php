...
function trouver_region($code) {
   global $regions;

   $regionTrouver = null;

   foreach($regions as $nom => $numDeptsReg) {
      if(in_array($code, $numDeptsReg)) {
         $regionTrouver = $nom;
      }
   }
...
<body>
   <table>
      ...
      <tbody>
         <?php
         foreach ($depts as $numDept => $dept){
            ?> 
               <tr> 
                  <td> <?php echo $numDept;?> </td>
                  <td> <?php echo htmlentities($dept[0]) . " (" . trouver_region($numDept) . ")";?> </td>
                  <td> <?php echo htmlentities($dept[1]);?> </td>
               </tr> 
            <?php
         }
         ?>
